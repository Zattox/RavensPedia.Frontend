...
function MatchesPage() {
    ...
    const handleAddMatch = async () => {
        const bestOf = prompt('Enter best of (number):');
        const maxTeams = prompt('Enter max number of teams:');
        const maxPlayers = prompt('Enter max number of players:');
        ...
        try {
            await api.post('/matches/', {
                ...
                max_number_of_teams: parseInt(maxTeams),
                max_number_of_players: parseInt(maxPlayers),
                tournament,
                date,
                description
            });
            ...
            const inProgressResponse = await api.get('/schedules/matches/get_in_progress/');
            setInProgressMatches(Array.isArray(inProgressResponse.data?.data) ? inProgressResponse.data.data : []);
            const scheduledResponse = await api.get('/schedules/matches/get_upcoming_scheduled/', {params: {num_matches: 50}});
            setScheduledMatches(Array.isArray(scheduledResponse.data?.data) ? scheduledResponse.data.data : []);
        } catch (error) {
