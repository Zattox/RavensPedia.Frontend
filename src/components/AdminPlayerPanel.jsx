...
function AdminPlayerPanel({ player_nickname, refreshPlayer }) { // Добавляем refreshPlayer
  ...
  const handleUpdatePlayer = async (values) => {
    ...
    if (Object.keys(updatedPlayer).length > 0) {
      try {
        ...
        showNotification('success', 'Успех!', 'Информация об игроке успешно обновлена.');
        await refreshPlayer(); // Перезапрашиваем данные игрока
  ...
  const handleDeletePlayer = async () => {
    try {
      ...
      showNotification('success', 'Успех!', 'Игрок успешно удален!');
